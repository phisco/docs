...
| api.sampleEmailBasedOIDCConfig | object | `{"allowedDomain":"","clientID":"","clientSecret":"","groupsClaim":"groups","issuerURL":"","providerName":"","requireVerifiedEmail":true,"usernameClaim":"email"}` | Sample OIDC identity-provider bootstrap. When issuerURL and clientID are set, an IdentityProvider YAML file is appended to the bootstrap config Secret. This provider: - drives the browser redirect login, unless global.demo.enabled is set,   in which case the demo keycloak provider keeps that role - redirects to the issuer's discovery-advertised authorization URL - uses the specified client ID and secret - authenticates user with username "&lt;providerName&gt;:&lt;usernameClaim value&gt;" - maps the groupsClaim to hub groups, prefixed with "&lt;providerName&gt;:" - (if requireVerifiedEmail) requires the email_verified claim to be true - (if allowedDomain is set) forces the username to end with @&lt;allowedDomain&gt; |
| api.sampleEmailBasedOIDCConfig.allowedDomain | string | `""` | Optional: restrict logins to a single email domain. Matches the mapped username, not the email claim, so it only means what it reads when usernameClaim is "email"; a provider login can carry a different domain or none at all, rejecting every user. Gate access in the provider instead. |
...
| api.sampleEmailBasedOIDCConfig.groupsClaim | string | `"groups"` | Token claim carrying the user's group membership. Group values are prefixed with "&lt;providerName&gt;:", so an OrganizationRoleBinding subject for the "admin" group is named "&lt;providerName&gt;:admin". Providers that publish groups under a non-standard claim need this overridden -- e.g. Amazon Cognito uses "cognito:groups". Set to "" to omit the mapping, which leaves only per-user role bindings working.  An absent claim is not an error: the user resolves with no groups, so group role bindings silently never match. Providers that withhold groups from the ID token must be configured provider-side -- Okta needs a groups claim on the authorization server set to "Always" for ID Token. |
...
| api.sampleEmailBasedOIDCConfig.requireVerifiedEmail | bool | `true` | Require the email_verified claim to be present and true. Set to false for providers that omit it: Okta serves it from /userinfo whenever an access token accompanies the ID token, which the authorization code flow always does, and Entra never emits it at all. This relaxes only the absent case -- with usernameClaim "email" the apiserver rejects email_verified: false on its own. |
| api.sampleEmailBasedOIDCConfig.usernameClaim | string | `"email"` | Token claim whose value becomes the hub username, prefixed with "&lt;providerName&gt;:". An absent claim fails authentication, so it must match what the provider emits. "email" covers Google, Keycloak, Okta, and Entra users with a mail attribute; use "preferred_username" where the provider login rather than the mail address is the identity. |
